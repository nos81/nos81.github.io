# noshnik.com — single-page (dark) edition
...
| `site.webmanifest`, `favicon.svg`, `og-image.svg` | Icons / social preview |
...
`sitemap.xml`, `robots.txt`, and a self-hosted `og-image.svg`. For the widest
social-preview support, export `og-image.svg` to a 1200×630 **PNG**
(`og-image.png`) and update the `og:image` / `twitter:image` URLs — some
crawlers (Twitter/X, Facebook) don't rasterise SVG previews.
